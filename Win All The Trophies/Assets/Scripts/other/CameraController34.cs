...
public class CameraController34 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (transform.position.x > 180f) // 카메라의 x좌표가 180보다 클 경우
        {
            transform.position = new Vector3(180f, transform.position.y, transform.position.z); // 카메라의 x좌표를 180로 하여 카메라의 x좌표가 180보다 오른쪽으로 가지 않도록 한다.
        }
