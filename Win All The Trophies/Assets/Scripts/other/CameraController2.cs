...
public class CameraController2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (transform.position.x > 150.1f) // 카메라의 x좌표가 150보다 클 경우
        {
            transform.position = new Vector3(150.1f, transform.position.y, transform.position.z); // 카메라의 x좌표를 150로 하여 카메라의 x좌표가 150보다 오른쪽으로 가지 않도록 한다.
        }
    }
}
