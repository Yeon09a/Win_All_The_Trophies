...
{
    static int trophyCount = ObjectCollision1.getTrophyCount(); // 트로피의 개수를 나타낸 변수. 다음 스테이지에 값을 전달하기 위해(다음 스테이지까지 트로피의 개수를 유지하기 위해) static으로 선언. 전 스테이지의 ObjectCollision1의 getTrophyCount함수를 통해 전 스테이지의 트로피의 개수를 가져옴.
    static int stage2trophy = 0; // 현재 스테이지에서만 얻은 트로피의 개수
    ...
    {
        // 적과 충돌
        if (collision.name.Contains("Enemy") || collision.name.Contains("Spikes")) //  Enemy(적) 또는 Spikes(함정)과 충돌하였을 때
        {
        ...
        if (collision.name.Contains("Trophy")) //  Trophy와 충돌하였을 때
        {
            ...
            stage2trophy++; // stage2trophy를 1 추가한다.(트로피의 개수를 하나 추가한다.)
        ...
        {
            SceneManager.LoadScene("Stage3"); // "Stage3" 씬으로 이동한다.
    ...
    // stage2trophy를 반환하는 getter
    static public int getStage2trophy() // 다른 스크립트에서 사용하기 위해 static으로 선언
    {
        return stage2trophy; // stage2trophy를 반환한다.
    }
