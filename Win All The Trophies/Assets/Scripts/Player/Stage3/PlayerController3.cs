...
public class PlayerController3 : MonoBehaviour
{
    ...
    bool lookRight = true; // Player의 방향을 구별하기 위해 만든 변수로, 오른쪽을 바라보고 있을 경우 true이고, 왼쪽을 바라보고 있을 경우 false이다.
    ...
    private void OnTriggerStay2D(Collider2D collision) // 충돌하는 동안(충돌 상태에 있을 때)
    {
        if (collision.name.Contains("Lava")) // Lava(용암)과 충돌하고 있을 때
        {
            if (lookRight == true) // player가 오른쪽을 보고 있을 때(lookRight가 true일 때)
            {
                transform.localScale = new Vector3(5, 5, 1); // Player의 이미지를 반전하여 오른쪽을 바라보도록 한다.
            }
            else // player가 왼쪽을 보고 있을 때(lookRight가 false일 때)
            {
                transform.localScale = new Vector3(-5, 5, 1); // Player의 이미지를 반전하여 왼쪽을 바라보도록 한다.
            }
            playerAnimator.SetTrigger("AttackedTrigger"); // AttackedTrigger로 바꿔주어 BeAttacked 애니메이션(player가 공격받는 애니메이션)이 출력되도록 한다.
