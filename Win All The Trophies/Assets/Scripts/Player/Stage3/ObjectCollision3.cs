...
{
    static int trophyCount = ObjectCollision2.getTrophyCount(); // 트로피의 개수를 나타낸 변수. 다음 스테이지에 값을 전달하기 위해(다음 스테이지까지 트로피의 개수를 유지하기 위해) static으로 선언. 전 스테이지의 ObjectCollision2의 getTrophyCount함수를 통해 전 스테이지의 트로피의 개수를 가져옴.
    static int stage3trophy = 0; // 현재 스테이지에서만 얻은 트로피의 개수
    ...
    {
        if (collision.name.Contains("Enemy") || collision.name.Contains("lavaball")) //  Enemy(적) 또는 lavaball(용암 볼)과 충돌하였을 때
        {
        ...
        if (collision.name.Contains("Trophy")) //  Trophy와 충돌하였을 때
        {
            ...
            stage3trophy++; // stage3trophy를 1 추가한다.(트로피의 개수를 하나 추가한다.)
        ...
        {
            SceneManager.LoadScene("Stage4"); // "Stage3" 씬으로 이동한다.
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        {
            playerHP.fillAmount -= 0.01f;
            playerSrc.PlayOneShot(attackedClip, 0.2f);
        }
    }
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.name.Contains("SpikeHead"))
        {
            playerHP.fillAmount -= 0.10f;
            playerSrc.PlayOneShot(attackedClip, 0.2f);
        }
    }
    ...
    // stage3trophy를 반환하는 getter
    static public int getStage3trophy() // 다른 스크립트에서 사용하기 위해 static으로 선언
    {
        return stage3trophy; // stage3trophy를 반환한다.
    }
}
